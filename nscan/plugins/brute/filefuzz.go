...
func reqPage(u string) (*page, *utils.Response, error) {
	page := &page{}
	...
	if req, err := utils.HttpRequset(u, method, "", false, header); err == nil {
		if utils.IntInSlice(req.StatusCode, []int{301, 302, 307, 308}) {
			page.is302 = true
		}
...
func FileFuzz(u string, indexStatusCode int, indexContentLength int, indexbody string) ([]string, []string) {
	...
	ch := make(chan struct{}, utils.Fuzzthreads)
	for _, payload := range filedic {
		var is404Page = false
		if errorTimes > 20 {
			return path, technologies
		}
		//if (utils.StringInSlice("/1.asp", path) && utils.StringInSlice("/1.jsp", path) && utils.StringInSlice("/2.jsp", path)) || (utils.StringInSlice("/1.php", path) && utils.StringInSlice("/1.jsp", path) && utils.StringInSlice("/2.jsp", path)) || (utils.StringInSlice("/zabbix/", path) && utils.StringInSlice("/grafana/", path) && utils.StringInSlice("/zentao/", path)) {
		//	return nil, nil
		//}
		if len(path) > 40 {
			return path, technologies
		...
		go func(payload string) {
			if url, req, err := reqPage(u + payload); err == nil {
				if url.is403 && (utils.SliceInString(url.title, page403title) || utils.SliceInString(req.Body, page403Content)) && !skip403 {
					path = append(path, u+payload)
					technologies = addfingerprints403(payload, technologies) // 基于403页面文件扫描指纹添加
				}
				if !utils.IntInSlice(req.StatusCode, page200CodeList) {
					is404Page = true
				}
				if url.isBackUpPath {
					if !url.isBackUpPage {
				...
				if utils.SliceInString(url.title, page404Title) {
					is404Page = true
				}
				if utils.SliceInString(req.Body, page404Content) {
					is404Page = true
				}
				...
				if url.is302 {
					...
					if utils.SliceInString(req.Location, location404) && utils.SliceInString(req.Location, payloadlocation404) {
						is404Page = true
					}
