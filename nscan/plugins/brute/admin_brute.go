...
func getinput(inputurl string) (usernamekey string, passwordkey string, loginurl string, ismd5 bool) {
	...
	if req, err := utils.HttpRequset(inputurl, "GET", "", true, nil); err == nil {
		...
		if hreflist != nil {
			...
			req, err = utils.HttpRequset(hrefurl.String(), "GET", "", true, nil)
			if err != nil {
				return "", "", "", false
			}
		}
...
func Admin_brute(u string) (username string, password string, loginurl string) {
	...
	if adminfalseurl, err := utils.HttpRequset(loginurl, "POST", adminfalsedata, false, nil); err == nil {
		...
		case 401:
			falseis401 = true
		case 200:
			if utils.SliceInString(adminfalseurl.Body, noaccount) {
				adminaccount = false
			}
			falseis200 = true
	...
	if testfalseurl, err := utils.HttpRequset(loginurl, "POST", testfalsedata, false, nil); err == nil {
		...
		case 401:
			falseis401 = true
		case 200:
			if utils.SliceInString(testfalseurl.Body, noaccount) {
				testaccount = false
			}
			falseis200 = true
	...
	for _, user := range usernames {
		for _, pass := range top100pass {
			...
			if req, err2 := utils.HttpRequset(loginurl, "POST", fmt.Sprintf("%s=%s&%s=%s", usernamekey, user, passwordkey, pass), false, nil); err2 == nil {
				if falseis401 {
					if req.StatusCode != 401 {
						log.Logger.Warn().Msg(fmt.Sprintf("Found vuln admin password|%s:%s|%s\n", user, pass, loginurl))
				...
				if falseis302 {
					...
					if req.Location != adminfalse302location && req.Location != testfalse302location {
						...
						if sucesstest, err := utils.HttpRequset(loginurl, "POST", sucesstestdata, false, nil); err == nil {
				...
				if falseis200 {
					if utils.SliceInString(req.Body, lockContent) {
						return "", "", ""
					}
