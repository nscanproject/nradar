...
func SeeyonFastjson(u string) bool {
	...
	if req, err := utils.HttpRequset(u+"/seeyon/main.do?method=changeLocale", "POST", data, false, nil); err == nil {
		if strings.Contains(req.Body, "com.alibaba.fastjson.JSONException:set") {
			log.Logger.Warn().Msg(fmt.Sprintf("Found vuln seeyon SeeyonFastjson|%s\n", u+"/seeyon/main.do?method=changeLocale"))
			return true
		}
	}
	return false
