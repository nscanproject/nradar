...
func CVE_2017_10271(url string) bool {
	...
	header["SOAPAction"] = ""
	if req, err := utils.HttpRequset(url+"/wls-wsat/CoordinatorPortType", "POST", post_str, false, header); err == nil {
		if (strings.Contains(req.Body, "<faultstring>java.lang.ProcessBuilder")) || (strings.Contains(req.Body, "<faultstring>0")) {
			log.Logger.Warn().Msg(fmt.Sprintf("Found vuln Weblogic CVE_2017_10271|%s\n", url))
			return true
		}
	}
	return false
