...
func Deduplication(slc []string) []string {
	if len(slc) < 1024 {
		return DeduplicationByLoop(slc)
	} else {
		return DeduplicationByMap(slc)
	}
}

func DeduplicationByLoop(slc []string) []string {
	result := []string{}
	for i := range slc {
		flag := true
		for j := range result {
...
func DeduplicationByMap(slc []string) []string {
	result := []string{}
